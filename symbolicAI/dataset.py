...
class Data():
  '''
  Arguments: 
   -> order = order upto which expansion is calculated
   -> types = List of the function to calculate their expansion
  '''
  def __init__(self, order, types=[]):
    super().__init__()
    self.order = order 
    self.types = types 
    self.vocab_to_int = None # Dict which store int value corresponding to tokens in vocab
    self.int_to_vocab = None # Dict which store tokens corresponding to int value
  ...
  def tokenize(self, df):
    '''
    Input : DataFrame to be tokenized
    Output : Tokenized DataFrame
    '''
